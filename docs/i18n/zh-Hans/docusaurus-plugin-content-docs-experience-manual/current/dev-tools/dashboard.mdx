...
### 创建XRExperience项目
进入我们刚刚创建的应用程序；同时通过右上方的菜单创建您的阴。您会看到一个创建项目的bootstrap屏幕，如下图所示。
...
- **UDM**: UDM是一个定制系统，允许用户通过XR组装自己的产品。这项功能不需要您写一行代码，只需从控制台上传一个3D模型或使用公共资源库来创造XR体验。
...
UDM项目被创建后，一样无法使用。您需要在XRCloud编辑器中上传您的模型才行，如下图所示。
...
在上传并设置好您的UDM模型后，您还需要通过XRMOD Cloud左上角FIle菜单中的Save菜单来保存项目，否则数据将不会被保存。
